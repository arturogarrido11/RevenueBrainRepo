...
export function CallsTable({ search = "", status = "all", dateRange = "all" }: CallsTableProps) {
  const calls = useQuery(api.calls.listFiltered, {
    ...
    dateRange: (dateRange as "all" | "today" | "7d" | "30d" | "month") === "all" ? undefined : dateRange as "today" | "7d" | "30d" | "month",
  })
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-0">
        <table className="w-full text-sm">
          ...
          <tbody>
            {calls === undefined ? (
              <tr><td colSpan={7} className="px-6 py-8 text-center text-sm text-muted-foreground">Loading calls…</td></tr>
            ) : calls.length === 0 ? (
              <tr><td colSpan={7} className="px-6 py-8 text-center text-sm text-muted-foreground">No calls match your filters.</td></tr>
            ) : calls.map((call) => (
                ...
                <td className="px-4 py-3 text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem><MessageSquare className="size-4" />Send SMS</DropdownMenuItem>
                      <DropdownMenuItem><Mail className="size-4" />Send Email</DropdownMenuItem>
                      <DropdownMenuItem><User className="size-4" />View Contact</DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem><CheckCircle className="size-4" />Mark as Handled</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </td>
              </tr>
