...
interface BookingResult {
  ...
  error?: string
  // Internal — sent to Convex at session end
  _internal?: SessionState["pendingAppointment"]
}
...
async function bookCalCom(
  ...
  const data = (await res.json()) as {
    data?: { uid?: string; title?: string; start?: string; end?: string }
  }
...
function captureAppointment(args: BookAppointmentArgs): BookingResult {
  return {
    success: true,
...
export const bookAppointmentDefinition = {
  ...
  description:
    "Book an appointment for the caller at a specific date and time. Always call check_availability first and confirm the time with the caller before calling this.",
  parameters: {
    type: "object",
    properties: {
      datetime_utc: {
        type: "string",
        description:
          "The appointment start time in UTC ISO 8601 format, e.g. 2026-09-08T14:00:00Z",
      },
      caller_name: {
        type: "string",
      ...
      datetime_raw: {
        type: "string",
        description:
          "The raw time the caller mentioned, e.g. 'next Tuesday at 2pm'. Used as fallback if UTC conversion is uncertain.",
      },
    },
