...
// ─────────────────────────────────────────────────────────────────────────────
async function fetchConfigFromConvex() {
...
// ─────────────────────────────────────────────────────────────────────────────
async function postSessionEnd(state) {
    const convexSiteUrl = process.env.CONVEX_SITE_URL;
    ...
    if (state.endCallSummary)
        payload.summary = state.endCallSummary;
    if (state.transcript)
        payload.transcript = state.transcript;
    if (state.capturedLead?.priority)
        payload.priority = state.capturedLead.priority;
    if (state.pendingAppointment)
        payload.appointment = state.pendingAppointment;
    try {
...
// ─────────────────────────────────────────────────────────────────────────────
export async function handleSession(twilioWs, callSid, fromNumber, toNumber, configOverride) {
    ...
    openaiWs.on("message", async (raw) => {
        ...
        if (eventType === "response.function_call_arguments.done") {
            ...
            const callId = msg.call_id;
            const name = msg.name;
            const argumentsStr = msg.arguments;
